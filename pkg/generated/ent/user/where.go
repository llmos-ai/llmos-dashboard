...
// HasModelfiles applies the HasEdge predicate on the "modelfiles" edge.
func HasModelfiles() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasModelfilesWith applies the HasEdge predicate on the "modelfiles" edge with a given conditions (other predicates).
func HasModelfilesWith(preds ...predicate.Modelfile) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newModelfilesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
