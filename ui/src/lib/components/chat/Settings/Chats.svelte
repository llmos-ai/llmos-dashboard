<script lang="ts">
  ...
  const exportChats = async () => {
    let blob = new Blob(
      [JSON.stringify(await getAllChats(localStorage.token))],
      {
  ...
  const exportAllUserChats = async () => {
    let blob = new Blob(
      [JSON.stringify(await getAllUserChats(localStorage.token))],
      {
        type: "application/json",
      }
    );
...
<div
  class="flex flex-col h-full justify-between space-y-3 text-sm max-h-[22rem]"
>
  <div class=" space-y-2">
    <div
