<script lang="ts">
  ...
  const getModels = async () => {
    ...
    selectedModel = await getDefaultImageGenerationModel(
      localStorage.token
    ).catch((error) => {
      return "";
    });
  ...
  const updateUrlHandler = async () => {
    if (imageGenerationEngine === "comfyui") {
      ...
      if (res) {
        ...
        ({ COMFYUI_BASE_URL } = await getImageGenerationEngineUrls(
          localStorage.token
        ));
      }
    } else {
      ...
      if (res) {
        ...
        ({ AUTOMATIC1111_BASE_URL } = await getImageGenerationEngineUrls(
          localStorage.token
        ));
      }
    }
  ...
  onMount(async () => {
    if ($user.role === "admin") {
      const res = await getImageGenerationConfig(localStorage.token).catch(
        (error) => {
          toast.error(error);
          return null;
        }
