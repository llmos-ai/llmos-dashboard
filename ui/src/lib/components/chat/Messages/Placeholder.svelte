<script lang="ts">
  ...
  $: modelfile =
    models[selectedModelIdx] in modelfiles
      ? modelfiles[models[selectedModelIdx]]
      : null;
...
{#if models.length > 0}
  <div class="m-auto text-center max-w-md px-2">
    ...
    <div
      class=" mt-2 mb-5 text-2xl text-gray-800 dark:text-gray-100 font-semibold"
    >
      {#if modelfile}
        ...
        {#if modelfile.user}
          <div
            class="mt-0.5 text-sm font-normal text-gray-500 dark:text-gray-500"
          >
            By <a href="https://openwebui.com/m/{modelfile.user.username}"
              >{modelfile.user.name
                ? modelfile.user.name
                : `@${modelfile.user.username}`}</a
            >
          </div>
        {/if}
      {:else}
