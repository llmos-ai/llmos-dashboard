<script lang="ts">
  ...
  const uploadDoc = async (file) => {
    const res = await uploadDocToVectorDB(localStorage.token, "", file).catch(
      (error) => {
        toast.error(error);
        return null;
      }
    ...
    documents.subscribe((docs) => {
      tags = docs.reduce((a, e, i, arr) => {
        return [
          ...new Set([
            ...a,
            ...(e?.content?.tags ?? []).map((tag) => tag.name),
          ]),
        ];
      }, []);
    });
...
{#if dragged}
  ...
  >
    <div
      class="absolute rounded-xl w-full h-full backdrop-blur bg-gray-800/40 flex justify-center"
    >
      ...
      {#if tags.length > 0}
        <div class="px-2.5 pt-1 flex gap-1 flex-wrap">
          <div class="ml-0.5 pr-3 my-auto flex items-center">
            <Checkbox
              state={filteredDocs.filter((doc) => doc?.selected === "checked")
                .length === filteredDocs.length
                ? "checked"
                : "unchecked"}
              indeterminate={filteredDocs.filter(
                (doc) => doc?.selected === "checked"
              ).length > 0 &&
                filteredDocs.filter((doc) => doc?.selected === "checked")
                  .length !== filteredDocs.length}
              on:change={(e) => {
                if (e.detail === "checked") {
