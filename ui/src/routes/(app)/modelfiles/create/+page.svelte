<script>
  ...
  $: tagName =
    title !== "" ? `${title.replace(/\s+/g, "-").toLowerCase()}:latest` : "";
...
${
  options.repeat_penalty !== ""
    ? `PARAMETER repeat_penalty ${options.repeat_penalty}`
    : ""
}
${
  options.repeat_last_n !== ""
    ? `PARAMETER repeat_last_n ${options.repeat_last_n}`
    : ""
}
${options.mirostat !== "" ? `PARAMETER mirostat ${options.mirostat}` : ""}
${
  options.mirostat_eta !== ""
    ? `PARAMETER mirostat_eta ${options.mirostat_eta}`
    : ""
}
${
  options.mirostat_tau !== ""
    ? `PARAMETER mirostat_tau ${options.mirostat_tau}`
    : ""
}
...
${
  options.num_predict !== ""
    ? `PARAMETER num_predict ${options.num_predict}`
    : ""
}
SYSTEM """${system}"""`.replace(/^\s*\n/gm, "");
  ...
  const submitHandler = async () => {
    ...
    if (
      ...
      (await getModelfileByTagName(localStorage.token, tagName).catch(
        () => false
      ))
    ) {
      toast.error(
  ...
  onMount(async () => {
    window.addEventListener("message", async (event) => {
      ...
      await tick();
      tagName = `${modelfile.user.username === "hub" ? "" : `hub/`}${
        modelfile.user.username
      }/${modelfile.tagName}`;
      desc = modelfile.desc;
...
<div
  class="min-h-screen max-h-[100dvh] w-full flex justify-center dark:text-white"
>
